...
export default function ThemaStudentHousing() {
    ...
    return (
        <BaseLayout>
            ...
            <SectionTitle title="Studentenhuisvesting"/>
            <Spacer spacing={4}/>
            {data.allContentfulGemeenteraadsverkiezingPartij.nodes.map((entry) => (
                ...
                <div style={{display: "flex", alignItems: "start"}}>
                    ...
                    {documentToReactComponents(JSON.parse(entry.standpuntStudentenhuisvesting.raw))}
                    </div>
                </div>
                <Spacer spacing={4}/>
                </>
...
const getAllData = graphql`
  query getGemeenteraadsverkiezingPartijHousingAll {
